...
import {
    deepEqual,
    primitiveArrayDeepEqual,
} from '@/common/utils/functionTools';
...
export class ManageBar extends React.PureComponent<{}, IManageBarState> {
    ...
    initIpc() {
        ipcRenderer.on(
            command.REPLY_LOAD_SUB_DIRECTORY_INFO,
            (
                event: IpcRendererEvent,
                data: { urls: string[]; flag: number }
            ) => {
                const { urls, flag } = data;
                if (flag === this.SYNC_FLAGS)
                    this.handleRecieveUrlsFromService(urls);
            }
        );
    }
    ...
    checkArrayIsInSameValue(arr: any[], ...path: string[]) {
        ...
        return arr.every((item) => {
            ...
            if (Array.isArray(checkItem) && Array.isArray(standardItem)) {
                return checkItem.every(
                    (ci, index) => ci === standardItem[index]
                );
            } else {
    ...
    initEvent() {
        EventHub.on(
            eventConstant.SHOW_MANAGE_BAR,
            this.loadPreferenceAndSubUrls
        );
    }
    ...
    loadPreferenceAndSubUrls = (data: string[]) => {
        ...
        this.setState({
            selectedUrls,
            normalModeLoading: 'loading',
            deepModeLoading: 'loading',
        });
        this.loadPreference(selectedUrls, 'normal');
        this.SYNC_FLAGS++;
        ipcRenderer.send(command.LOAD_SUB_DIRECTORY_INFO, {
            urls: selectedUrls,
            flag: this.SYNC_FLAGS,
        });
    };
    ...
    loadPreference = async (urls: string[], mode: 'deep' | 'normal') => {
        ...
        const urlsData: IDirectoryData[] = (await db.directory
            .find({ $or: querys })
            .exec()) as any[];
        if (urlsData?.length > 0) {
            if (urlsData.length < urls.length) {
                ...
                urlsData.forEach((urlData) => {
                    readingMode =
                        readingMode === urlData.readingMode
                            ? readingMode
                            : '_different';
                    readingDirection =
                        readingDirection === urlData.readingDirection
                            ? readingDirection
                            : '_different';
                    scrollModeDirection =
                        scrollModeDirection === urlData.scrollModeDirection
                            ? scrollModeDirection
                            : '_different';
                    pageReAlign =
                        pageReAlign === urlData.pageReAlign
                            ? pageReAlign
                            : '_different';
                    tag = primitiveArrayDeepEqual(tag, urlData.tag)
                        ? tag
                        : '_different';
                    author = primitiveArrayDeepEqual(author, urlData.author)
                        ? author
                        : '_different';
                });
        ...
        this.setState(
            setStateObj as Pick<IManageBarState, keyof IManageBarState>
        );
    };

    getUrls = () =>
        this.state.deepMode ? this.state.urls : this.state.selectedUrls;
    ...
    handleAuthorsChange = async (authors: string[]) => {
        ...
        if (this.state.deepMode)
            this.setState({
                deepData: { ...this.state.deepData, authors },
                deepModeLoading: false,
            });
        else
            this.setState({
                normalData: { ...this.state.normalData, authors },
                authorSelectorIsShow: false,
                normalModeLoading: false,
            });
    ...
    handleTagsChange = async (tags: string[]) => {
        ...
        if (this.state.deepMode)
            this.setState({
                deepData: { ...this.state.deepData, tags },
                deepModeLoading: false,
            });
        else
            this.setState({
                normalData: { ...this.state.normalData, tags },
                tagSelectorIsShow: false,
                normalModeLoading: false,
            });
    ...
    setReadingDirection = async (value: any) => {
        ...
        if (this.state.deepMode)
            this.setState({
                deepData: { ...this.state.deepData, readingDirection: value },
                deepModeLoading: false,
            });
        else
            this.setState({
    ...
    setReadingMode = async (value: any) => {
        ...
        if (this.state.deepMode)
            this.setState({
                deepData: { ...this.state.deepData, readingMode: value },
                deepModeLoading: false,
            });
        else
            this.setState({
                normalData: { ...this.state.normalData, readingMode: value },
                normalModeLoading: false,
            });
    };
    ...
    setPageReAlign = async (value: any) => {
        ...
        if (this.state.deepMode)
            this.setState({
                deepData: { ...this.state.deepData, pageReAlign: value },
                deepModeLoading: false,
            });
        else
            this.setState({
                normalData: { ...this.state.normalData, pageReAlign: value },
                normalModeLoading: false,
            });
    };
    ...
    getValue = <K extends keyof manageData>(key: K): manageData[K] =>
        this.state.deepMode
            ? this.state.deepData[key]
            : this.state.normalData[key];
        ...
        return (
            <div className={style.info}>
                {this.state.deepMode ? (
                    ...
                    <h2 className={style.infoItem}>
                        {t('%selectedDirNum%').replace(
                            '%{selectedDirNum}',
                            selectedDirNum.toString()
                        )}
                    </h2>
                )}
                <Button
                    onMouseEnter={() => {
                        hintMainText(t('%deepModeDesc%'));
                    }}
    ...
    renderCategory = () => {
        ...
        if (isArray(authors)) {
            if (authors.length > 0) {
                authorsMessage = authors
                    .reduce((preVal, curVal) => `${preVal}, ${curVal}`, '')
                    .slice(2);
                if (authorsMessage.length > this.MAX_MESSAGE_LENGTH) {
                    authorsMessage =
                        authors.length > 1
                            ? `${authorsMessage.slice(0, 10)}...${t(
                                  '%totallyAuthorCount%'
                              )}`.replace(
                                  '%{totallyAuthorCount}',
            ...
            <div className={style.group}>
                ...
                <div className={style.item}>
                    ...
                    <div
                        ...
                        onClick={() => {
                            this.setState({ tagSelectorIsShow: true });
                        }}
                    >
                        {tagsMessage}
                    </div>
                ...
                <div className={style.item}>
                    ...
                    <div
                        className={style.selectorButton}
                        onClick={() => {
                            this.setState({ authorSelectorIsShow: true });
                        }}
                    >
                        {authorsMessage}
                    </div>
                ...
                <div className={style.item}>
                    ...
                    <div>
                        <Select
                            ...
                            placeholder={
                                isUndefinedOrNull(scrollModeDirection)
                                    ? t('%notSettingYet%')
                                    : t('%differentSetting%')
                            }
                        >
                ...
                <div className={style.item}>
                    ...
                    <div>
                        <Select
                            ...
                            placeholder={
                                isUndefinedOrNull(readingDirection)
                                    ? t('%notSettingYet%')
                                    : t('%differentSetting%')
                            }
                        >
    ...
    renderLoadingModal = () => {
        ...
        const loadingType = this.state.deepMode
            ? this.state.deepModeLoading
            : this.state.normalModeLoading;
        return (
            <div className={style.loadingModal}>
                ...
                <span className={style.loadingText}>
                    {loadingType === 'saving' ? t('%saving%') : t('%loading%')}
                </span>
            </div>
        );
    };
    ...
    render(): JSX.Element {
        ...
        const isLoading = this.state.deepMode
            ? this.state.deepModeLoading
            : this.state.normalModeLoading;

        const mainContent = (
            <div
                className={`${style.scrollWrapper} medium-scrollbar`}
                style={{ filter: isLoading !== false ? 'blur(4px)' : '' }}
            >
                <Info />
                <Category />
                <ReadingSettings />
