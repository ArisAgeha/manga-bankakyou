...
export class ConfigurationService {
    ...
    upadteUserConfig(
        newConfigs: Array<{ id: string; key: string; value: any }>
    ) {
        newConfigs.forEach((config) => {
            ...
            if (!configType.includes(getTypeof(value))) {
                this.logService.warn(
                    `value "${value}" has the wrong type, ${id}-${key} is required "${configType}"`
                );
                return;
            }
            ...
            if (
                item.value === value ||
                (!item.hasOwnProperty('value') && item.default === value)
            )
                return;
    ...
    private _writeConfigToFile(id: string): void {
        ...
        Object.entries(this._config[id]).forEach((item) => {
            ...
            if (
                _config.hasOwnProperty('value') &&
                _config.value !== _config.default
            )
                moduleConfig.properties[_key] = _config.value;
        });
    ...
    private _loadUserConfigs(): void {
        const dirInfo: string[] = this.fileService.getDirInfoSync(
            this.userConfigDir
        );
        if (!dirInfo) return;
        dirInfo.forEach((filename: string) => {
            const fileContent: IUserConfig = this.fileService.loadJsonSync(
                this.userConfigDir,
                filename
            ) as IUserConfig;
    ...
    private _insertUserConfig(
        configId: IUserConfig['id'],
        userConfig: IUserConfig['properties']
    ) {
...
export interface IUserConfig {
    id: string;
    properties: {
        [key: string]: ConfigItemType | undefined;
    };
}
